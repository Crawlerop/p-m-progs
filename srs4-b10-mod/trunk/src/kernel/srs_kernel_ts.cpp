...
// Custom flags
#define TS_INITIAL_PID 0x100
#define INVERT_PMT 1
#define TS_ALIGN_FRAMES 1
//#define PICASA 1
...
SrsTsContext::SrsTsContext()
{
    ...
    stream_pid = TS_INITIAL_PID;    
    //srs_trace("initial: %d",stream_pid);
    aud_pid = 0;
    vid_pid = 0;
    pat_cc = 0;
    pmt_cc = 0;
...
srs_error_t SrsTsContext::align_frames(ISrsStreamWriter* writer) {
    srs_error_t err = srs_success;
    #ifdef TS_ALIGN_FRAMES
    if (vid_pid) {
        SrsTsChannel *vdp_ch = get(vid_pid);
        if (vdp_ch && (vdp_ch->continuity_counter & 0xf) != 0) {
            if ((err = af_padding(writer, vdp_ch, vid_pid)) != srs_success) {
                return err;
            }
        }
    }
    ...
    if (aud_pid) {
        ...
        if (aud_ch && (aud_ch->continuity_counter & 0xf) != 0) {
            if ((err = af_padding(writer, aud_ch, aud_pid)) != srs_success) {
                return err;
            }
        } 
    }
...
srs_error_t SrsTsContext::encode(ISrsStreamWriter* writer, SrsTsMessage* msg, SrsVideoCodecId vc, SrsAudioCodecId ac)
{
    ...
    switch (vc) {
        case SrsVideoCodecIdAVC:
            ...
            //srs_trace("video: %d",stream_pid);
            //srs_trace("video pid: %d",vid_pid);
            if (!vid_pid) vid_pid = stream_pid++;
            video_pid = vid_pid;
            break;
    ...
    switch (ac) {
        case SrsAudioCodecIdAAC:
            ...
            //srs_trace("audio: %d",stream_pid);
            //srs_trace("audio pid: %d",aud_pid);
            if (!aud_pid) aud_pid = stream_pid++;
            audio_pid = aud_pid;
            break;
...
SrsTsPacket* SrsTsPacket::null_af_packet(SrsTsContext* context, int16_t pid, uint8_t cc) {
...
srs_error_t SrsTsContext::af_padding(ISrsStreamWriter* writer, SrsTsChannel *chan, int16_t pid) {
    srs_error_t err = srs_success;
    while ((chan->continuity_counter & 0xf) != 0) {
        SrsTsPacket* pkt = SrsTsPacket::null_af_packet(this, pid, chan->continuity_counter++);
...
SrsTsPacket* SrsTsPacket::create_pmt(SrsTsContext* context,
    ...
    if (vs == SrsTsStreamVideoH264) {
        //srs_trace("vpid: %d", vpid);
        //if (!pmt->PCR_PID) pmt->PCR_PID = vpid;
    ...
    if (as == SrsTsStreamAudioAAC || as == SrsTsStreamAudioMp3) {
        ...
        //srs_trace("apid: %d", apid);
        //if (!pmt->PCR_PID) pmt->PCR_PID = apid;
...
srs_error_t SrsTsContextWriter::align_frames() {
    return context->align_frames(writer);
}
